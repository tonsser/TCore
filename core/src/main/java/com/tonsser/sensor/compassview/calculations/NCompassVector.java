package com.tonsser.sensor.compassview.calculations;


public class NCompassVector {
    private final float[] matrixArray = new float[9];

    private volatile float x = 0f;
    private volatile float y = 0f;
    private volatile float z = 0f;

    public NCompassVector() {
        this(0, 0, 0);
    }

    public NCompassVector(NCompassVector v) {
        this(v.x, v.y, v.z);
    }

    public synchronized float getX() {
        return x;
    }

    public synchronized void setX(float x) {
        this.x = x;
    }

    public synchronized float getY() {
        return y;
    }

    public synchronized void setY(float y) {
        this.y = y;
    }

    public synchronized float getZ() {
        return z;
    }

    public synchronized void setZ(float z) {
        this.z = z;
    }

    /**
     * Set the vector's values.
     *
     * @param x float x value.
     * @param y float y value.
     * @param z float z value.
     */
    public NCompassVector(float x, float y, float z) {
        set(x, y, z);
    }

    /**
     * Get the vector's values.
     * <p/>
     * array[0] = x;
     * array[1] = y;
     * array[2] = z;
     *
     * @param array float array representing this vector.
     */
    public synchronized void get(float[] array) {
        if (array == null || array.length != 3)
            throw new IllegalArgumentException("get() array must be non-NULL and size of 3");

        array[0] = this.x;
        array[1] = this.y;
        array[2] = this.z;
    }

    /**
     * Set the vector from a given vector.
     *
     * @param v Vector to copy values form.
     */
    public void set(NCompassVector v) {
        if (v == null) return;

        set(v.x, v.y, v.z);
    }

    /**
     * Set the vector's values.
     * <p/>
     * array[0] = x;
     * array[1] = y;
     * array[2] = z;
     *
     * @param array float array representing this vector.
     */
    public void set(float[] array) {
        if (array == null || array.length != 3)
            throw new IllegalArgumentException("get() array must be non-NULL and size of 3");

        set(array[0], array[1], array[2]);
    }

    /**
     * Set the Vector to the given values.
     *
     * @param x float x value.
     * @param y float y value.
     * @param z float z value.
     */
    public synchronized void set(float x, float y, float z) {
        this.x = x;
        this.y = y;
        this.z = z;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public synchronized boolean equals(Object obj) {
        if (obj == null) return false;

        NCompassVector v = (NCompassVector) obj;
        return (v.x == this.x && v.y == this.y && v.z == this.z);
    }

    public synchronized boolean equals(float x, float y, float z) {
        return (this.x == x && this.y == y && this.z == z);
    }

    /**
     * Vector addition
     */
    public synchronized void add(float x, float y, float z) {
        this.x += x;
        this.y += y;
        this.z += z;
    }

    /**
     * Vector addition
     */
    public void add(NCompassVector v) {
        if (v == null) return;

        add(v.x, v.y, v.z);
    }

    /**
     * Vector subtraction
     */
    public void sub(float x, float y, float z) {
        add(-x, -y, -z);
    }

    /**
     * Vector subtraction
     */
    public void sub(NCompassVector v) {
        if (v == null) return;

        add(-v.x, -v.y, -v.z);
    }

    /**
     * Scalar multiplication
     */
    public synchronized void mult(float s) {
        this.x *= s;
        this.y *= s;
        this.z *= s;
    }

    /**
     * Scalar division
     */
    public synchronized void divide(float s) {
        this.x /= s;
        this.y /= s;
        this.z /= s;
    }

    /**
     * Length of this vector
     */
    public synchronized float length() {
        return (float) Math.sqrt(this.x * this.x + this.y * this.y + this.z * this.z);
    }

    /**
     * Length of this vector, projected upon the Y-plane
     */
    public synchronized float length2D() {
        return (float) Math.sqrt(this.x * this.x + this.z * this.z);
    }

    /**
     * Normalize (length = 1.0)
     */
    public void norm() {
        divide(length());
    }

    /**
     * Scalar product
     */
    public synchronized float dot(NCompassVector v) {
        if (v == null) return 0f;

        return this.x * v.x + this.y * v.y + this.z * v.z;
    }

    /**
     * Vector product
     */
    public synchronized void cross(NCompassVector u, NCompassVector v) {
        if (v == null || u == null) return;

        float x = u.y * v.z - u.z * v.y;
        float y = u.z * v.x - u.x * v.z;
        float z = u.x * v.y - u.y * v.x;
        this.x = x;
        this.y = y;
        this.z = z;
    }

    /**
     * Matrix product
     */
    public synchronized void prod(NCompassMatrix m) {
        if (m == null) return;

        m.get(matrixArray);
        float xTemp = matrixArray[0] * this.x + matrixArray[1] * this.y + matrixArray[2] * this.z;
        float yTemp = matrixArray[3] * this.x + matrixArray[4] * this.y + matrixArray[5] * this.z;
        float zTemp = matrixArray[6] * this.x + matrixArray[7] * this.y + matrixArray[8] * this.z;

        this.x = xTemp;
        this.y = yTemp;
        this.z = zTemp;
    }


    /**
     * {@inheritDoc}
     */
    @Override
    public synchronized String toString() {
        return "<" + this.x + ", " + this.y + ", " + this.z + ">";
    }
}
