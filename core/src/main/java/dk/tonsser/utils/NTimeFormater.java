...
public class NTimeFormater {
    ...
    public static String toTimeSincePretty(Date timeSince, String justNow, String aMinuteAgo, String minutesAgo, String anHourAgo,
        ...
        if (diff < NUnits.MIN_IN_MS) {
            return justNow;
        } else if (diff < 2 * NUnits.MIN_IN_MS) {
            return aMinuteAgo;
        } else if (diff < 50 * NUnits.MIN_IN_MS) {
            return diff / NUnits.MIN_IN_MS + " " + minutesAgo;
        } else if (diff < 90 * NUnits.MIN_IN_MS) {
            return anHourAgo;
        } else if (diff < 120 * NUnits.MIN_IN_MS) {
            return "2 " + hoursAgo;
        } else if (diff < 24 * NUnits.HOUR_IN_MS) {
            return diff / NUnits.HOUR_IN_MS + " " + hoursAgo;
        } else if (diff < 48 * NUnits.HOUR_IN_MS) {
            return yesterday;
        } else {
            return diff / NUnits.DAY_IN_MS + " " + daysAgo;
        }
    }
}
