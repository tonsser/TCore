...
public class NHeapController {
	...
	public static boolean isCurrentHeapLow(float percent,float percentLowHeap) {
		...
		if((heapPercent>percent && leftAmount<15)||(maxHeap<24 && heapPercent>percentLowHeap))
			return true;
		else
			return false;
	}
	...
	public static boolean isHeapLowScreenTakenIntoAccount() {
		try{
			if(maxHeap<DEFINITION_OF_SMALL_HEAP && NScreenParameters.screenDensityConstant>1)
				return true;
			else
				return false;
		}
	...
	public boolean isHeapLow() {
		...
		if(maxHeap<DEFINITION_OF_SMALL_HEAP)
			return true;
		else
			return false;
	}
	 ...
	 */
	public static void takeCoolDownIfNeeded(LowHeapAsyncListener mLowHeapAsyncListener){
		if(isCurrentHeapLow()) {
	 ...
	 * it's cancel-method will be called whether or not the heap was actually cleared
	 */
	public static void takeCoolDownIfNeeded(int ms, LowHeapAsyncListener mLowHeapAsyncListener){
		if(isCurrentHeapLow()) {
