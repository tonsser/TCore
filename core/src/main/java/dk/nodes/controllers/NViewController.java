...
import dk.nodes.utils.NViewPadding;

public class NViewController {
	...
	public static NViewPadding getPaddings(View mView){
		return new NViewPadding(mView.getPaddingTop(), mView.getPaddingTop(), mView.getPaddingLeft(), mView.getPaddingRight());
	}
	
	public static void setPaddings(View mView, NViewPadding mNViewPadding){
		mView.setPadding(mNViewPadding.getLeft(), mNViewPadding.getTop(), mNViewPadding.getRight(), mNViewPadding.getBottom());
	}
	...
	public static void forceCenterText(Button... button){
		...
		for (Button mButton : button) {
			...
			if ((drawables != null) && (drawables.length > 0) && (drawables[0] != null || drawables[2] != null)) {
				// fetch drawableLeft
				Drawable drawableLeft = (Drawable) drawables[0];
				Drawable drawableRight = (Drawable) drawables[2];
