...
/**
 * Extend your MainActivity (Splash screen) with this. And you will get a perfect splash screen timer.
 * Override onSplashScreenOver and startNextActivity in there while finishing this.
 * @author Casper Rasmussen - 2013
 *
 */
public abstract class NBaseSplashActivity extends NBaseActivity implements NApiAsyncListener, NBaseSplashTaskDoInBackground{
	...
	protected void onStart() {
        ...
        loadData(); // CHNT 02-16-2015 : moved from onCreate() to onStart() - allows settings to be changed before the async is started.
		...
		if(!loaded){
			if(NBaseApplication.getInstance().DEBUG && clickToContinueInDebug){
				...
				addContentView(debugView,new RelativeLayout.LayoutParams( LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
				initDebugOverlay();
				setDebugOverlay();
				loaded = true;
