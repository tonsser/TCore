 ...
 * 
 *
 *  NRotationDetector rotator = new NRotationDetector(this, new NRotationDetector.RotationListener() {
 ...
 * 
*/
public class NRotationDetector extends OrientationEventListener {

	private static final String TAG = NRotationDetector.class.getSimpleName();
	...
	public NRotationDetector(Context context, RotationListener rotationListener) {
		super(context);
	...
	private static void measure(Context context) {
		...
		if( context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
			isPortrait = true;
			if(r%2 == 0) { // even numbers means "horizontal" relative to the "natural orientation"
				isTablet = false;
			}
			else {
				isTablet = true;
	...
	public void onOrientationChanged(int orientation) {
		...
		if(!NUtils.isOutdated(delayBeforeFirstCallbackMs, enabledTimeStamp))
			return;
		if(isPortrait) {
