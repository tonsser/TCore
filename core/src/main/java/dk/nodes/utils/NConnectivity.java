...
public class NConnectivity {
	...
	private static NBasicAlertDialog showNotConnectedPopupIfNotConnectionDialog;
	...
	public static void showNotConnectedPopupIfNotConnection(final Activity mActivity){
		if(!isConnected(mActivity) && (showNotConnectedPopupIfNotConnectionDialog == null || !showNotConnectedPopupIfNotConnectionDialog.isShowing())){
			showNotConnectedPopupIfNotConnectionDialog = new NBasicAlertDialog(mActivity,null, "No internet connection",
					"Ignore", "Go to settings", true, new NBasicAlertDialogListener() {
						
						@Override
						public void onRightBtnClicked() {
							NAndroidIntents.toWIFISettings(mActivity);
						}
	...
	public static boolean isConnectedFast(Context context){
		...
		return (info != null && info.isConnected() && NConnectivity.isConnectionFast(info.getType(),info.getSubtype()));
	}
	...
	public static boolean isConnectionFast(int type, int subType){
		if(type== ConnectivityManager.TYPE_WIFI){
			...
			case NConnectivity.NETWORK_TYPE_EHRPD:
				return true; // ~ 1-2 Mbps
			case NConnectivity.NETWORK_TYPE_EVDO_B:
				return true; // ~ 5 Mbps
			case NConnectivity.NETWORK_TYPE_HSPAP:
				return true; // ~ 10-20 Mbps
			case NConnectivity.NETWORK_TYPE_IDEN:
				return false; // ~25 kbps 
			case NConnectivity.NETWORK_TYPE_LTE:
				return true; // ~ 10+ Mbps
				// Unknown
