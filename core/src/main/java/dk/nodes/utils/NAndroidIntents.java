...
import dk.nodes.controllers.web.NTagHandler;
import dk.nodes.widgets.dialogs.alert.NBasicAlertDialog;
import dk.nodes.widgets.dialogs.alert.NBasicAlertDialog.NBasicAlertDialogListener;

public class NAndroidIntents {

	private static String TAG = NAndroidIntents.class.getName();
	...
	public static void toMarket(Context mContext){
		toMarket(mContext,NBuild.getPackageName(mContext));		
	}
	...
	public static void toBrowser(Context mContext, String url){
		if(mContext == null || url == null){
			NLog.d(TAG+" toBrowser", "Context or url was null");
			return;
		}
		try {
			...
			NLog.e(TAG+" toBrowser url: "+url, e);
		}
	}
	...
	public static void callNumberAfterConfirmation(final Activity mActivity, final String phoneNumbe,
		...
		new NBasicAlertDialog(mActivity, null, question,no, yes, true, new NBasicAlertDialogListener() {
	...
	public static void toLocationSettings(final Context mContext){
		try {
			...
			NLog.e(TAG+ " toLocationSettings",e);
		}
	}
	...
	public static void sendMailWithHTML(final Context mContext, String mailTo, String subject, String mailAsHTML, String chooseTitle){
		try {
			Intent emailIntent = new Intent(Intent.ACTION_SENDTO,
					Uri.fromParts("mailto", mailTo, null));
			emailIntent.putExtra(Intent.EXTRA_SUBJECT, subject);
			emailIntent.putExtra(Intent.EXTRA_TEXT, Html.fromHtml(mailAsHTML, null, new NTagHandler()));
			mContext.startActivity(Intent.createChooser(emailIntent, chooseTitle).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
		} catch (Exception e) {
			NLog.e(TAG+" sendMail",e);
		}
